 ...
 */
	public function testAssociationLazyLoading() {
		...
		$this->assertTrue(isset($Article->belongsTo['Categories']));
		$this->assertFalse(property_exists($Article, 'Categories'));
		$this->assertTrue(isset($Article->Category));
		$this->assertInstanceOf('Categories', $Article->Category);
 ...
 */
	public function testGetAssociated() {
		...
		$Article->bindModel(array('hasMany' => array('Categories')));
		$this->assertEquals(array('Comment', 'Categories'), $Article->getAssociated('hasMany'));

		$results = $Article->getAssociated();
		$results = array_keys($results);
		sort($results);
		$this->assertEquals(array('Categories', 'Comment', 'Tag', 'User'), $results);

		$Article->unbindModel(array('hasAndBelongsToMany' => array('Tag')));
		$this->assertEquals(array(), $Article->getAssociated('hasAndBelongsToMany'));

		$result = $Article->getAssociated('Categories');
		$expected = array(
 ...
 */
	public function testAutoConstructAssociations() {
		...
		$expected = array(
			...
			'Categories' => array(
				'className' => 'Categories', 'foreignKey' => 'category_id',
				'conditions' => '', 'fields' => '', 'order' => '', 'counterCache' => ''
			)
		);
		...
		$this->assertEquals('Categories', $TestModel->Category->name);
		$this->assertEquals('Categories', $TestFakeModel->Category->name);
